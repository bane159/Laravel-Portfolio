...
window.onload = () => {
    // Add blur event listener for the name field
    const fields = [
        { name: 'name', regex: /^[A-Za-zČčĆćŽžŠšĐđ\s]{2,50}$/, errorMessage: "Name must only contain letters (including Serbian Latin letters), spaces, and must not exceed 50 characters." },
        { name: 'email', regex: /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/, errorMessage: "A valid email is required." },
        { name: 'phone', regex: /^\d{5,12}$/, errorMessage: "Phone is required and must not exceed 12 characters." },
        { name: 'company_name', regex: /^[A-Za-zČčĆćŽžŠšĐđ\s]{0,100}$/, errorMessage: "Company name must not exceed 255 characters." },
        { name: 'website', regex: /^(https?:\/\/)?([\w\d\-]+\.)+\w{2,}(\/.*)?$|^$/, errorMessage: "Website must be a valid URL and must start with http or https" },
        { name: 'exact_budget', regex: /^.{0,100}$/, errorMessage: "Exact budget must not exceed 100 characters." },
        { name: 'timeline', regex: /^.{0,255}$/, errorMessage: "Timeline must not exceed 255 characters." },
        { name: 'message', regex: /^.{1,2000}$/, errorMessage: "Message must not exceed 2000 characters." },
        { name: 'interest', regex: /^(Branding|Web Design|App Design|Other|Hire me)$/, errorMessage: "Interest is required and must be one of: Branding, Web Design, App Design, Other." },
        { name: 'budget_range', regex: /^(< 10k|10k - 30k|30k - 50k|50k - 70k|> 70k)$/, errorMessage: "Please Choose a budget." }
    ];

    fields.forEach(field => {
        const inputElement = document.querySelector(`input[name="${field.name}"], textarea[name="${field.name}"], input[type="radio"][name="${field.name}"]`);
        if (inputElement) {
            inputElement.addEventListener('blur', function () {
                const value = this.value.trim();
                const errorElementId = `invalid-${field.name}-blur`;
                let errorElement = document.querySelector(`#${errorElementId}`);

                // Remove existing error and styles
                ...
                if (value === '') {
                    ...
                    errorElement = document.querySelector(`#invalid-${field.name}`);
                    if (errorElement) {
                        errorElement.textContent = "";
                        errorElement.classList.remove('is-invalid', 'is-valid');
                    }
                    inputElement.classList.remove('is-invalid', 'is-valid');
                }
                else if (!field.regex.test(value)) {
                    // Add error message and red outline
                    this.classList.add('is-invalid');
                    errorElement = document.querySelector(`#invalid-${field.name}`);
                    if (errorElement) {
                    ...
                    // Add green outline for valid input
    ...
    document.getElementById('form-contact').addEventListener('submit', function(e) {
        ...
        let budget_range = $('input[name="budget_range"]:checked').val();

        console.log(budget_range); // Check if the value is correctly retrieved
        const interestElement = document.querySelector('input[name="interest"]:checked');
        let interest = interestElement ? interestElement.value : null;
        console.log(interest); // Check if the value is correctly retrieved
        
    
        let exact_budget = $("#exact_budget").val();
        ...
        fields.forEach(field => {
            let value;
            if (field.name === 'budget_range' || field.name === 'interest') {
                value = $(`input[name="${field.name}"]:checked`).val() || '';
            } else {
                value = $(`input[name="${field.name}"], textarea[name="${field.name}"]`).val().trim();
            }
            console.log( `field ${field.name} did: ${field.regex.test(value)} with value: ${value}`);
            if (!field.regex.test(value)) {
                errors.push({name: field.name, errorMessage: field.errorMessage});
        ...
        if (errors.length > 0) {
            ...
            errors.forEach(error => {
                let errorField = document.querySelector(`input[name="${error.name}"]`);
                if (!errorField) {
                    errorField = document.querySelector(`textarea[name="${error.name}"]`);
                }
                if (errorField) {
                    errorField.classList.add('is-invalid');
                    let errorElement = document.querySelector(`#invalid-${error.name}`);
                    if (errorElement) {
            ...
            // Loop through the errors array and display messages
            errors.forEach(error => {
                console.log("Empty field: " + error.name);
                let errorElement = document.querySelector(`#invalid-${error.name}`);
                if (errorElement) {
                    errorElement.textContent = error.errorMessage;
                    errorElement.classList.add('d-block');
                }
                const inputElement = document.querySelector(`input[name="${error.name}"]`);
                if (inputElement) {
                    inputElement.classList.add("is-invalid");
                }
                const textareaElement = document.querySelector(`textarea[name="${error.name}"]`);
                if (textareaElement) {
        ...
        if(errors.length === 0){
            ...
            $.ajax({
                url: '/contact/send', // The endpoint where the data will be sent
                ...
                error: function(xhr) {
                    ...
                    for (let field in data.errors) {
                        if (data.errors.hasOwnProperty(field)) {
                            ...
                            document.querySelector("#invalid-" + field).textContent = data.errors[field][0];
                            const inputElement = document.querySelector(`input[name="${field}"]`);
                            if (inputElement) {
                                inputElement.classList.add("is-invalid");
                            }
                            const textareaElement = document.querySelector(`textarea[name="${field}"]`);
